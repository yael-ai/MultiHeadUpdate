...
class AbstractAgent_Local(Configurable, ABC):

    def __init__(self, config=None):
        super(AbstractAgent_Local, self).__init__(config)
    ...
    def plan(self, state, count):
        ...
        """
        return [self.act(state,count)]

    def plan_test(self, state):
        """
            Plan an optimal trajectory from an initial state.
...
class AbstractStochasticAgent(AbstractAgent_Local):
    """
